$(function () {
    ...
    function runFakeTerminal() {
      ...
      term.writeln([
        'This is the JangsTerminal! This is the backend of your app.'
      ].join('\n\r'));
  
      term.writeln('Type `help` to get what to do.');
      addDecoration(term);
      prompt(term);
    ...
    var commands = {
      help: {
        f: () => {
          term.writeln([
            'This is the JangsTerminal. Welcome!',
            '',
            ...Object.keys(commands).map(e => `  ${e.padEnd(10)} ${commands[e].description}`)
          ].join('\n\r'));
          prompt(term);
        },
        description: 'This prints the Jangs Command List',
      },
      ls: {
        f: () => {
          term.writeln(['app.css', 'console.js', 'app.html', 'console.html', 'console.js', 'app.js'].join('\r\n'));
          term.prompt(term);
        },
        description: 'Prints your JangS apps directory'
      },
      loadtest: {
        f: () => {
          let testData = [];
    ...
    function runCommand(term, text) {
      ...
      if (command.length > 0) {
        ...
        term.writeln(`${command}: command not found in JangS terminal database. Try making one yourself by editing the console.js!`);
      } 
      prompt(term);
    }
  ...
  function addDecoration(term) {
    ...
    decoration.onRender(element => {
      ...
      element.innerText = 'You can create hyperlinks by making an addon!';
